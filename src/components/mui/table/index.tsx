...
// Type Imports
import type { Order, Row, RowKey, MuiTableProps } from './types'

export type { MuiTableProps } from './types'
...
export default function MuiTable<T extends Row>(props: MuiTableProps<T>) {
  const {
    rows,
    sortBy,
    headCells,
  ...
  const columnCount = useMemo(() => headCells.length + (multiple ? 1 : 0), [headCells, multiple])
    ...
    <div className='border-bs'>
      ...
      <TableContainer>
        <Table aria-labelledby='tableTitle' size={'medium'}>
          ...
          />
          <TableBody>
            {visibleRows.length <= 0 ? <MuiTableEmptyState col={columnCount} /> : tableRows()}
            {emptyRows > 0 && (
              <TableRow
                style={{
                  height: 70 * emptyRows
      ...
      <TablePaginationStyled
        rowsPerPageOptions={rowsPerPageOptions ?? [10, 15, 25]}
        component='div'
        count={total}
